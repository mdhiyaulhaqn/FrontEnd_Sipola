<template>
    <div class="row">
        <div class="col-12">
            <b-breadcrumb id="breadcrumb">
                <b-breadcrumb-item :to="{name: 'purchase-order'}">
                    Purchase Order List
                </b-breadcrumb-item>
                <b-breadcrumb-item active>
                    Detail Purchase Order
                </b-breadcrumb-item>
            </b-breadcrumb>

            <div class="judul">
                <strong>Detail Purchase Order</strong>
            </div>

            <card>
                <div class="container-fluid">
                    <b-row>
                        <div class="company-name">
                            <strong>{{ purchaseOrder.company.nama }}</strong>
                        </div>
                    </b-row>

                    <b-row>
                        <div class = "col-md-2 col-6">PO No</div>
                        <div class = "col-md-5 col-6">: {{ purchaseOrder.noPurchaseOrder }}</div>
                        <div class = "col-md-2 col-6">Cust. Ref. No</div>
                        <div class = "col-md-3 col-6">: {{ purchaseOrder.noCustRef }}</div>
                    </b-row>

                    <b-row>
                        <div class = "col-md-2 col-6">PO Date</div>
                        <div class = "col-md-5 col-6">: {{ purchaseOrder.datePurchaseOrder.split("T")[0].split("-").reverse().join('-') }}</div>
                        <div class = "col-md-2 col-6">Project No</div>
                        <div class = "col-md-3 col-6">: {{ purchaseOrder.noProject }}</div>
                    </b-row>

                    <b-row>
                        <div class = "col-md-2 col-6">Attn.</div>
                        <div class = "col-md-5 col-6">: {{ purchaseOrder.attnName }}</div>
                        <div class = "col-md-2 col-6">Created By</div>
                        <div class = "col-md-3 col-6">: {{ purchaseOrder.createdBy }}</div>
                    </b-row>

                    <b-row>
                        <div class = "col-md-2 col-6">Address</div>
                        <div class = "col-md-5 col-6">: {{ purchaseOrder.company.alamat }}</div>
                        <div class = "col-md-2 col-6">Created At</div>
                        <div class = "col-md-3 col-6">: {{ purchaseOrder.createdAt.split("T")[0].split("-").reverse().join('-') }}</div>
                    </b-row>

                    <b-row>
                        <div class = "col-lg-6 col-sm-4 col-12"><br>
                            Purchased Items
                        </div>
                        <div class = "col-lg-6 col-sm-8 col-12">
                            <button  @click="downloadReport" id="download_button" class="btn btn-primary">
                                Download
                                <span class="ti-download"></span>
                            </button>
                        </div>
                    </b-row>    
                    <br>

                    <b-row>
                        <b-col >
                            <div class="tabel-service">
                                <div slot="raw-content" class="table-responsive" style="font-size:12px">
                                    <b-table 
                                    :items="purchaseOrder.purchasedItems" 
                                    :fields="fields"
                                    >
                                        <template v-slot:cell(No)="row" id="price">
                                            {{purchaseOrder.purchasedItems.indexOf(row.item) + 1}}
                                        </template>
                                        <template v-slot:cell(Total Price(IDR))="row" id="price">
                                            {{row.item.pricePerUnit}} * {{row.item.quantity}}
                                        </template>
                                    </b-table>
                                    <br>
                                    <b-row align-content="right">
                                        <div class="col-md-7"></div>
                                        <div class = "col-md-2 col-6">
                                            <strong>Sub-total</strong>
                                        </div>
                                        <div class = "col-md-3 col-6">
                                            {{this.purchaseOrder.sub_total_price}}
                                        </div>
                                    </b-row>
                                    <b-row align-content="rigt">
                                        <div class="col-md-7"></div>
                                        <div class = "col-md-2 col-6">
                                            <strong>PPN (10%)</strong>
                                        </div>
                                        <div class = "col-md-3 col-6">
                                            {{this.purchaseOrder.price_ppn}}
                                        </div>
                                    </b-row>
                                    <b-row align-content="right">
                                        <div class="col-md-7"></div>
                                        <div class = "col-md-2 col-6">
                                            <strong>Total Price</strong>
                                        </div>
                                        <div class = "col-md-3 col-6">
                                            {{this.purchaseOrder.total_all_price}}
                                        </div>
                                    </b-row>
                                </div>
                            </div>
                        </b-col>
                    </b-row>

                    <b-row>
                        <div class="col-12" v-html=purchaseOrder.paymentNote></div>
                    </b-row>

                    <b-row>
                        <div class="col">
                            <br>
                            <button v-b-modal.modal-delete id ="delete-button" class="btn btn-primary">
                                Delete
                            </button>
                            <router-link :to="{name: 'update-purchase-order'}">
                                <button id ="edit-button" class="btn btn-primary">
                                    Edit
                                </button>
                            </router-link>
                        </div>
                    </b-row>
                </div>
            </card>
        </div>

        <!-- Modal Delete Confirmation -->
        <b-modal id="modal-delete" ref="modal-delete" centered>
            <template v-slot:modal-title>
                <div class="container">
                    <h5 id="modal-title-delete">Delete Purchase Order?</h5>
                </div>
            </template>

            <template v-slot:default>
                <div class="container">
                    <b-row>
                        <b-col class="modal-icon col-2">
                            <img src="@/assets/img/delete-confirm-icon.png" alt="" width="50px">
                        </b-col>
                        <b-col class="col-10">
                            <p id="modal-message"> Purchase Order no {{purchaseOrder.noPurchaseOrder}} will be removed from the list.</p>
                        </b-col>
                    </b-row>
                </div>
            </template>

            <template v-slot:modal-footer="{ cancel }">
                <b-col class="button-confirm-group">
                    <b-button @click="onSubmit" id ="confirm_delete_button" variant="outline-danger">
                        Yes, delete it
                    </b-button>
                    <b-button @click="cancel()" id ="cancel_delete_button" class="btn btn-danger">
                        Cancel
                    </b-button>
                </b-col>
            </template>
        </b-modal>

        <!-- Modal Delete Success -->
        <b-modal id="modal-success" ref="modal-success" centered v-model="successModal" @ok="redirect()">
            <template v-slot:modal-title>
                <div class="container">
                    <h5 id="modal-title-success">Success!</h5>
                </div>
            </template>
            <template v-slot:default>
                <div class="container">
                    <b-row>
                        <b-col class="modal-icon col-2">
                            <img src="@/assets/img/success-icon.png" alt="" width="50px">
                        </b-col>
                        <b-col class="col-10">
                            <p id="modal-message">Purchase Order no. {{purchaseOrder.noPurchaseOrder}} was successfully deleted from list.</p>
                        </b-col>
                    </b-row>
                </div>
            </template>
            <template v-slot:modal-footer="{ ok }">
                <b-col class="button-confirm-group">
                    <b-button @click="ok()" id="ok-button" variant="outline-primary">
                        OK
                    </b-button>
                </b-col>
            </template>
        </b-modal>

        <!-- Modal Download -->
        <b-modal id="modal-download" ref="modal-download" centered>
            <template v-slot:modal-title>
                <div class="container">
                    <h5 id="modal-title-download">Download Purchase Order</h5>
                </div>
            </template>
            <template v-slot:default>
                <div class="container">
                    <b-row>
                        <b-col class="modal-icon col-2">
                            <img src="@/assets/img/download-icon.png" alt="" width="50px">
                        </b-col>
                        <b-col class="col-10">
                            <p id="modal-message"> The system is downloading purchase order no. {{purchaseOrder.noPurchaseOrder}}</p>
                        </b-col>
                    </b-row>
                </div>
            </template>
            <template v-slot:modal-footer="{ ok }">
                <b-col class="button-confirm-group">
                    <b-button @click="ok()" id="ok-button" variant="outline-primary">
                        OK
                    </b-button>
                </b-col>                
            </template>
        </b-modal>
    </div>
</template>

<script>
import axios from 'axios';
import jsPDF from 'jspdf';
import * as autoTable from 'jspdf-autotable';
import html2canvas from "html2canvas";

export default {
    data() {
        return {
            purchaseOrder : '',
            successModal : false,
            fields: [
                {key: 'id', label: 'No', sortable: true},
                {key: 'item', label: 'Item', sortable: true},
                {key: 'uom', label: 'UOM', sortable: true},
                {key: 'quantity', label: 'Quantity', sortable: true},
                {key: 'pricePerUnit', label: 'Unit Price (IDR)', formatter: value => {
                    return value.toLocaleString('id-ID', {maximumFractionDigits:2})
                }},
                {key: 'total_price', label: 'Total Price (IDR)', formatter: value => {
                    return value.toLocaleString('id-ID', {maximumFractionDigits:2})
                }},
            ]
        };
    },

    beforeMount(){
        this.getDetail();
    },

    methods:{
        onSubmit(evt) {
            evt.preventDefault();
            this.deletePurchaseOrder(JSON.stringify(this.purchaseOrder));
        },

        showMessage(status){
            this.successModal = true;
        },

        computePrice(){
            var sub_total_price = 0;
            for(let i=0; i < this.purchaseOrder.purchasedItems.length; i++){
                this.purchaseOrder.purchasedItems[i].id = i+1;
                this.purchaseOrder.purchasedItems[i].total_price = this.purchaseOrder.purchasedItems[i].pricePerUnit * this.purchaseOrder.purchasedItems[i].quantity;
                sub_total_price += this.purchaseOrder.purchasedItems[i].total_price;
            }

            this.purchaseOrder.sub_total_price = sub_total_price;
            this.purchaseOrder.price_ppn = 0.1 * this.purchaseOrder.sub_total_price;
            this.purchaseOrder.total_all_price = (this.purchaseOrder.sub_total_price + this.purchaseOrder.price_ppn).toLocaleString('id-ID', {maximumFractionDigits:2});

            this.purchaseOrder.sub_total_price = this.purchaseOrder.sub_total_price.toLocaleString('id-ID', {maximumFractionDigits:2});
            this.purchaseOrder.price_ppn = this.purchaseOrder.price_ppn.toLocaleString('id-ID', {maximumFractionDigits:2});
        },

        getDetail: function(){
            axios.get('http://localhost:8080/api/purchase-order/' +this.$route.params.id)
            .then(res => {this.purchaseOrder = res.data, this.computePrice()})
            .catch(err => this.purchaseOrder = err.data);
        },

        deletePurchaseOrder(purchaseOrder){
            axios.put('http://localhost:8080/api/purchase-order/' + this.$route.params.id + '/delete', 
            purchaseOrder, 
                { headers: {
                    'Content-Type': 'application/json',
                }
            })
            .then(res => {this.showMessage(res.data.status)});
        },

        redirect(){
            this.$router.push({ name: 'purchase-order' })
        },

        hideModal(){
            this.$refs['modal-download'].hide();
        },

        cancel(){
            this.$refs['modal-delete'].hide();
        },

        downloadReport:function(){
            var doc = new jsPDF()
            let noPurchaseOrder = this.purchaseOrder.noPurchaseOrder;
            let datePurchaseOrder = this.purchaseOrder.datePurchaseOrder.split("T")[0].split("-").reverse().join('-');
            let noCustRef = this.purchaseOrder.noCustRef;
            let noProject = this.purchaseOrder.noProject;
            let companyName = this.purchaseOrder.company.nama;
            let attnName = this.purchaseOrder.attnName;
            let companyAddr = this.purchaseOrder.company.alamat;
            let paymentNote = this.purchaseOrder.paymentNote;

            var splitAddr = doc.splitTextToSize(companyAddr, 170);
            var splitNote = doc.splitTextToSize(paymentNote, 240);

            var logo = new Image()
            logo.src = "data:image/jpeg;base64,"

            // starting point
            let startX = 20
            let startY = 20

            // header
            doc.addImage(logo, 'png', startX, 10, 20, 20);
            doc.setFontStyle("bold")
            doc.setFont("muli")
            doc.setFontSize(14)
            doc.setTextColor('#348FBA')
            doc.text('PT SixAB Tehnik Industri', startX+25, 15)

            doc.setFontSize(8)
            doc.setTextColor('#000000')
            doc.setFontStyle("light")
            doc.setFont("times new roman")
            doc.text("Jl. Raya Puspitek-Parung Km. 7, Taman Sari Bukit Damai", startX+25, startY)
            doc.text("Phone: (0251) 861-7096", 190, startY, "right")
            doc.text("Pedurenan Gunung Sindur, Bogor 16340", startX+25, startY+5)
            doc.text("Fax: (0251) 861-7097", 190, startY+5, "right")
            doc.text("NPWP No: 66.607.817.5-403.000", startX+25, startY+10)

            doc.line(startX, startY+15, startX+170, startY+15)

            // title
            doc.setFontStyle("bold")
            doc.setFontSize(12)
            doc.text("PURCHASE ORDER", startX+65, startY+23)
            doc.line(startX+65, startY+26, startX+105, startY+26)
            doc.setFontStyle("normal")
            doc.setFontSize(10)
            doc.text(noPurchaseOrder, startX+65, startY+32)

            // left column
            doc.setFontStyle("bold")
            doc.text("To:", startX, startY+37)
            doc.text(companyName, startX, startY+42)
            doc.setFontStyle("normal")
            doc.text(splitAddr, startX, startY+47)
            doc.setFontStyle("bold")
            doc.text("Attn. "+attnName, startX, startY+65)
            
            // right column
            doc.text("Date", 130, startY+37)
            doc.setFontStyle("normal")
            doc.text(": "+datePurchaseOrder, 152, startY+37)
            doc.setFontStyle("bold")
            doc.text("Proj. No", 130, startY+42)
            doc.setFontStyle("normal")
            doc.text(": "+noProject, 152, startY+42)
            doc.setFontStyle("bold")
            doc.text("Cust. Ref. No", 130, startY+47)
            doc.setFontStyle("normal")
            doc.text(": "+noCustRef, 152, startY+47)

            // purchased items table
            var purchasedItems = this.purchaseOrder.purchasedItems

            var column = [
                {title: 'No', dataKey: 'id'},
                {title: 'Item', dataKey: 'deskripsi'},
                {title: 'Quantity', dataKey: 'quantity'},
                {title: 'UOM', dataKey: 'uom'},
                {title: 'Unit Price(IDR)', dataKey: 'pricePerUnit'},
                {title:  'Total Price(IDR)', dataKey: 'total_price'},
            ]

            doc.autoTable(column, purchasedItems, {
                startY: startY+70,
                margin: 20,
                headStyles: {
                    fillColor: [218, 37, 28],
                    textColor: [255, 255, 255], //White
                    align: "center"
                },
                bodyStyles: {
                    textColor: [0, 0, 0], //White
                    fontSize: 8,
                },
                theme: 'striped',
            });

            // final point
            let finalY = doc.autoTable.previous.finalY;

            // price
            var subTotal = this.purchaseOrder.sub_total_price.toLocaleString('id-ID', {maximumFractionDigits:2})
            var ppn = this.purchaseOrder.price_ppn.toLocaleString('id-ID', {maximumFractionDigits:2})
            var grandTotal = this.purchaseOrder.total_all_price.toLocaleString('id-ID', {maximumFractionDigits:2})

            doc.setFontStyle("bold")
            doc.text("Sub-total", 130, finalY+10)
            doc.setFontStyle("normal")
            doc.text(": Rp", 160, finalY+10)
            doc.text(subTotal, 190, finalY+10, "right")

            doc.setFontStyle("bold")
            doc.text("PPN (10%)", 130, finalY+15)
            doc.setFontStyle("normal")
            doc.text(": Rp", 160, finalY+15)
            doc.text(ppn, 190, finalY+15, "right")

            doc.setFontStyle("bold")
            doc.text("Total Price", 130, finalY+20)
            doc.setFontStyle("normal")
            doc.text(": Rp", 160, finalY+20)
            doc.text(grandTotal, 190, finalY+20, "right")

            doc.fromHTML(paymentNote, startX, finalY+30,{
                pagesplit: true,
                'widht':165
            })

            // signage
            doc.text("Issued and Approved by", startX, finalY+100)
            doc.text("Confirm and Accepted by:", 128, finalY+100)
            doc.setFontStyle("bold")
            doc.text("PT SixAB Tehnik Industri", startX, finalY+105)
            doc.setFontStyle("normal")
            doc.text("Date:", 128, finalY+105)
            doc.setFontStyle("bold")
            doc.text("(Evi Yuliana)", startX, finalY+135)
            doc.text("(........................................)", 128, finalY+135)

            
            doc.save("Purchase Order No." + noPurchaseOrder + " - " + companyName + ".pdf")
            this.$refs['modal-download'].show();
        }
    }
};
</script>

<style scoped>
body {
    font-family: 'Muli', sans-serif;
    background: #fafafa;
}

.judul{
    text-align: center;
    color: black;
    font-size:20px;
    margin-bottom: 20px;
}
.company-name{
    color: black;
    text-decoration: black;
    font-size:20px;
    margin-bottom: 20px;
}
.tabel-service{
    font-size:15px;
}
#download_button{
    font-size: 10px;
    float:right;
    background-color: green;
    color:white;
    border-color: white;
}
#modal-delete{
    color:black;
}
.ti-download{
    margin-left:10px;
    margin-right: 10px;
}
.fa-warning{
    margin-left:10px;
    margin-right: 10px;
}
.modal-header{
    background-color: #FF3E1D;
}
.button_back{
    background-color: #FF3E1D;
    color:white;
    border-color: white;
    float:right;
    margin-top: 40px;
}
.button_oke{
    background-color:white;
    color:#FF3E1D;
    border-color: #FF3E1D;
    float:right;
    margin-top: 40px;
}
.ok-button{
    color:#109CF1;
    border-color:#109CF1;
    background-color: white;
}
#manage-button{
    margin-left: auto;
    margin-right: auto;
}

.button-group{
    text-align: center;
}
button{
    border-radius: 8px;
}
#delete-button{
    font-size: 10px;
    width: 56;
    background-color: #ff3e1d;
    color:white;
    border-color: white;
}
#edit-button{
    font-size: 10px;
    width: 130px;
    background-color: #109CF1;
    color:white;
    border-color: white;
}
.ti-trash{
    font-size: 50px;
    text-align: center;
    color:#ff3e1d;
}
.button-confirm-group{
    text-align: right;
}
#confirm_delete_button{
    font-size: 10px;
    width: 130px;
    border-color: #ff3e1d;
    border-width: 1px;
    margin-right: 10px;
}
#cancel_delete_button{
    font-size: 10px;
    background-color: #ff3e1d;
    color:white;
    border-color: white;
    border-width: 1px;
}
#breadcrumb{
  font-size: 12px;
  /* text-decoration: underline; */
  margin: -35px 0 -5px -15px;
  color: #FF3E1D;
  background: none;
}
h5{
    margin-bottom: -4px;
}
#modal-message{
    font-size: 16px;
}
#modal-title-delete{
    color:#FF3E1D;
    font-weight: 1000;
}
#modal-title-download{
    color: #109CF1;
    font-weight: 1000;
}
#modal-title-success{
    color: #109CF1;
}
#price{
    text-align: center;
}
</style>
